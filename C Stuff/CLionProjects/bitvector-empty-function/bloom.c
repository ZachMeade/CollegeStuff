...
{
  struct bloom *newBloom = malloc(sizeof(struct bloom));
  newBloom->bit_vector = malloc(sizeof(bitset));
...
{
    int hashedNumber = hash_string(item, BLOOM_HASH1, this->size);
    if(bitset_lookup(this->bit_vector, hashedNumber)==1){
        return 1;
    }
    else
        return 0;
...
{
    int hashedNumber = hash_string(this, BLOOM_HASH1, this->size);
    bitset_add(this->bit_vector, hashedNumber);
}
