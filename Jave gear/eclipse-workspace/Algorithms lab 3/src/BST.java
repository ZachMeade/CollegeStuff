...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private String printKeysInOrder(Node focusNode){
        String string = "";
        string += "(";
        if(isEmpty()) return "()";
        else if(size(focusNode)==1) return "(()"+focusNode.key+ "())";
        else{
            if(focusNode !=null){
                string+= printKeysInOrder(focusNode.left);
                string+= (focusNode.key);
                string+= printKeysInOrder(focusNode.right);
            }
            string += ")";
            return string;
        }
